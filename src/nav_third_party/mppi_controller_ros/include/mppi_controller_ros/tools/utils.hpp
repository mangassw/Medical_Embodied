...
#ifndef MPPI_CONTROLLER_ROS__TOOLS__UTILS_HPP_
#define MPPI_CONTROLLER_ROS__TOOLS__UTILS_HPP_
  ...
  auto applyFilterOverAxis =
    ...
    const float hist_0, const float hist_1, const float hist_2, const float hist_3) -> void
    {
      ...
      for (unsigned int idx = 0; idx != num_sequences; idx++) {
        Eigen::Array<float, 9, 1> window;
        window << pt_m4, pt_m3, pt_m2, pt_m1, pt, pt_p1, pt_p2, pt_p3, pt_p4;
        sequence(idx) = applyFilter(window);
        pt_m4 = pt_m3;
        pt_m3 = pt_m2;
        pt_m2 = pt_m1;
        pt_m1 = pt;
...
#endif  // MPPI_CONTROLLER_ROS__TOOLS__UTILS_HPP_
