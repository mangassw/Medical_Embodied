    ...
    Parameters * parameters)
{
  ...
  critic_manager_.on_configure(node_, private_node_, name_, costmap_ros_, parameters_);
  noise_generator_.initialize(settings_, isHolonomic(), name_, parameters_);
...
void Optimizer::getParams()
{
  ...
  auto & s = settings_;
  s.model_dt = parameters_->optimizer_model_dt;
  s.time_steps = parameters_->optimizer_time_steps;
  s.batch_size = parameters_->optimizer_batch_size;
  s.iteration_count = parameters_->optimizer_iteration_count;
  s.temperature = parameters_->optimizer_temperature;
  s.gamma = parameters_->optimizer_gamma;
  s.retry_attempt_limit = parameters_->optimizer_retry_attempt_limit;
  s.base_constraints.vx_max = parameters_->vx_max;
  s.base_constraints.vy = parameters_->vy_max;
  s.base_constraints.vx_min = parameters_->vx_min;
  s.base_constraints.wz = parameters_->wz_max;
  s.base_constraints.ax_max = parameters_->ax_max;
  s.base_constraints.ax_min = parameters_->ax_min;
  s.base_constraints.ay_max = parameters_->ay_max;
  s.base_constraints.az_max = parameters_->az_max;
  s.sampling_std.vx = parameters_->vx_std;
  s.sampling_std.vy = parameters_->vy_std;
  s.sampling_std.wz = parameters_->wz_std;
  ...
  motion_model_name = parameters_->optimizer_motion_model;

  s.constraints = s.base_constraints;
  setMotionModel(motion_model_name);

  double controller_frequency = parameters_->controller_frequency;
...
geometry_msgs::TwistStamped Optimizer::getControlFromSequenceAsTwist(const ros::Time & stamp)
{
...
void Optimizer::setMotionModel(const std::string & model)
{
  if (model == "DiffDrive") {
    ...
    motion_model_ = std::make_shared<AckermannMotionModel>(parameters_, name_);
  } else {
